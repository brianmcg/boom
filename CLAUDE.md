...
The project is being converted to TypeScript, and the destination is all of it. `core/physics` is done; the rest follows. Write new files as `.ts`, and convert a `.js` file being worked on substantially rather than extending it.

Two documents hold the parts worth reading before changing anything:

- **`src/App/components/Game/core/physics/types.ts`** — the module's conventions. Where a type lives, how a class is written, what `private`/`protected`/`readonly` each mean and when to reach for them, when an accessor is justified, and how `null` and `undefined` are spelled differently on purpose.
- **`test/physics/README.md`** — the three harnesses, when `equivalence` is allowed to fail, and the shims and divergences it already carries.
