...
export default class WorldBodies {
  ...
  static createEnemy({
    ...
    const spatterColorOffset =
      bloodColors.indexOf(bloodColor) * numberOfSpatters;
  ...
  static createWorld({ scene, data, graphics }) {
    ...
    const grid = data.grid.reduce(
      (cols, col) => [
        ...
        col.reduce(
          (cells, cell) => [
            ...cells,
            WorldBodies.createCell({ cell, soundSprite, props }),
          ],
          []
        ),
      ],
