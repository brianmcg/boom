  ...
  const enemies = data.enemies.reduce((memo, enemy) => {
    ...
    return [
      ...
      new ENEMIES[props.enemies[enemy.name].behaviour]({
        ...
        width: Math.min(Math.max(Math.round(CELL_SIZE * enemy.width), CELL_SIZE / 2), CELL_SIZE - 1),
        length: Math.min(Math.max(Math.round(CELL_SIZE * enemy.length), CELL_SIZE / 2), CELL_SIZE - 1),
